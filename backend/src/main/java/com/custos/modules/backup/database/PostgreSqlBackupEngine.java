...
public class PostgreSqlBackupEngine implements DatabaseBackupEngine {
    ...
    public List<String> buildCommandList(DatabaseBackupRequest request) {
        ...
        if (request.getUsername() != null && !request.getUsername().trim().isEmpty()) {
            ...
            cmd.add("--username=" + request.getUsername().trim());
        }

        cmd.add("--format=p"); // Plain text SQL for streaming compression
        cmd.add("--clean");
        cmd.add("--if-exists");
        cmd.add("--no-owner");
        cmd.add("--no-privileges");

        cmd.add("--dbname=" + request.getDatabaseName().trim());
    ...
    public BackupResult executeBackup(DatabaseBackupRequest request, File targetFile) {
        ...
        if (request.getPassword() != null && !request.getPassword().isEmpty()) {
            env.put("PGPASSWORD", request.getPassword());
        }
        ...
        try {
            ...
            log.info("Starting PostgreSQL dump for database: {} -> {}", request.getDatabaseName(), targetFile.getAbsolutePath());
            process = pb.start();
            ...
            if (!finished) {
                ...
                throw new RuntimeException("pg_dump timed out after " + timeoutSeconds + " seconds");
            }
            ...
            if (exitCode != 0) {
                ...
                log.error("pg_dump exited with error code {}: {}", exitCode, errorMsg);
                throw new RuntimeException("pg_dump failed (code " + exitCode + "): " + errorMsg);
            }
            ...
            log.info("PostgreSQL dump completed successfully: {} bytes, SHA-256: {} in {}ms",
            ...
            log.error("PostgreSQL dump execution error: {}", e.getMessage());
            throw new RuntimeException("PostgreSQL backup failed: " + e.getMessage(), e);
        }
    }
